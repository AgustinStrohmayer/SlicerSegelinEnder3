...
class CanvasContainer(QWidget):
    ...
    def apply_ruler_palette(self, tokens) -> None:  # type: ignore[no-untyped-def]
        c = tokens.color
        self.h_ruler.set_palette(tick=c.border_strong, text=c.muted, accent=c.accent)
        self.v_ruler.set_palette(tick=c.border_strong, text=c.muted, accent=c.accent)
        # ruler backgrounds picked up via QSS through #Ruler objectName
